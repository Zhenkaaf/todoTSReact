...
Итак, ReturnType<typeof store.getState> означает, что мы берем тип возвращаемого значения функции store.getState(),
то есть тип текущего состояния хранилища.

Тип RootState используется для указания типа корневого состояния Redux во всем приложении. Он обычно экспортируется и
используется в других частях кода для обеспечения типизации и доступа к состоянию хранилища в различных частях приложения. */
